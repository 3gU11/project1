    ...
    <el-drawer v-model="editVisible" title="信息强改" size="400px">
      <el-form v-if="editingUnit" label-width="80px" size="small">
        ...
        <el-form-item label="机型">
          <el-select v-model="editForm.model_type" filterable style="width:100%">
  ...
  try {
    ...
    if (Array.isArray(list)) {
      for (const item of list) {
        const mt = typeof item === 'string'
          ? item.trim()
          : String(item?.model_type || item?.model_name || '').trim()
        if (!mt) continue
        const mf = String(item?.model_family || '').trim()
        if (mt && mf) nextMap[mt.toUpperCase()] = mf
        if (!['G', 'XS', 'AUTO', 'SPECIAL'].includes(mt.toUpperCase())) nextTypes.push(mt)
      }
    }
    modelTypes.value = [...new Set(nextTypes)]
...
const editModelTypes = computed(() => modelTypes.value)
...
async function saveEdit() {
  ...
  saving.value = true
  try {
    const { contract_no: _, ...editableFields } = editForm.value
