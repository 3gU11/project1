  ...
  <div class="machine-edit">
    ...
    <van-cell-group inset title="结构化拍照任务" class="mt-4">
      ...
      <div v-for="task in tasks" :key="task.id" class="task-card">
        ...
        <div v-if="shouldShowRecognitionPanel(task)" class="ocr-panel">
          <div class="ocr-title">二维码识别结果</div>
          <div v-for="field in task.ocr_results" :key="field.id || field.field_code" class="ocr-field">
            ...
            <div class="ocr-meta">
              ...
              <span v-if="field.recognition_source === 'qr_static'">二维码</span>
              <span v-else>人工输入</span>
            </div>
            <div v-if="field.display_value" class="combined-code">
...
type QrCandidateOptions = {
  cropRatio: number
  maxDimension: number
  filter?: string
}

const canvasToQrFile = (canvas: HTMLCanvasElement, fileName: string) => new Promise<File>((resolve, reject) => {
  canvas.toBlob((blob) => {
    if (!blob) {
      reject(new Error('二维码图片预处理失败'))
      return
    }
...
const createQrCandidate = async (
  bitmap: ImageBitmap,
  fileName: string,
  { cropRatio, maxDimension, filter = 'none' }: QrCandidateOptions,
) => {
  const cropSize = Math.min(bitmap.width, bitmap.height) * cropRatio
  const sourceWidth = cropRatio < 1 ? cropSize : bitmap.width
  const sourceHeight = cropRatio < 1 ? cropSize : bitmap.height
  ...
  return canvasToQrFile(canvas, `qr-${fileName.replace(/\.[^.]+$/, '')}.jpg`)
}

const createQrCandidates = async (image: File) => {
  const candidates: File[] = [image]
  let bitmap: ImageBitmap | null = null
  try {
    bitmap = await createImageBitmap(image)
    const options: QrCandidateOptions[] = [
      { cropRatio: 1, maxDimension: 1200 },
      { cropRatio: 1, maxDimension: 850 },
      { cropRatio: 0.82, maxDimension: 900 },
      { cropRatio: 0.68, maxDimension: 850 },
      { cropRatio: 0.82, maxDimension: 900, filter: 'grayscale(1) contrast(1.35)' },
    ]
    for (const option of options) {
      candidates.push(await createQrCandidate(bitmap, image.name, option))
    }
  } catch {
    // The unmodified image remains available when the browser cannot preprocess it.
  } finally {
    bitmap?.close()
  }
  return candidates
...
const scanQrFromPhoto = async (file: Blob, fileName: string) => {
  ...
  if (!qrModulePrepared) {
    ...
    qrModulePrepared = true
  }
  const candidates = await createQrCandidates(image)
  for (const candidate of candidates) {
    try {
      const results = await readBarcodes(candidate, {
        formats: ['QRCode'],
        tryHarder: true,
        tryRotate: true,
        tryInvert: true,
      ...
      const values = [...new Set(results.map((result) => String(result.text || '').trim()).filter(Boolean))]
      if (values.length > 1) return { value: '', ambiguous: true }
      if (values.length === 1) return { value: values[0], ambiguous: false }
    } catch {
      // Try the next scale/crop before using the server-side QR decoder.
    }
  }
  ...
  try {
    ...
    if (task.ocr_enabled) {
      if (!qrValue) {
        ...
        qrResult = { value: qrValue, ambiguous: !!serverQr?.ambiguous }
        if (qrResult.ambiguous) {
          qrValue = ''
          showToast('检测到多个二维码，请重新拍摄或人工确认')
        }
      }
      if (qrValue) {
        await inventoryApi.savePhotoTaskRecognition(task.id, {
          source: 'qr_static',
          value: qrValue,
          field_code: 'component_serial_no',
          field_name: '标签编码',
        })
        showSuccessToast('二维码已识别，请确认编号')
      } else {
        showToast('未识别到二维码，请重新拍摄或手工输入标签编码')
      }
    } else {
...
const deleteTaskPhoto = async (task: PhotoTask) => {
  try {
    await showConfirmDialog({
      title: '清空这个拍照项？',
      message: '会删除当前照片和识别结果，并取消该位置已确认的编号绑定。清空后可重新拍照。',
...
const submitWarningLines = () => {
  ...
  if (summary.value.ocr_pending > 0) {
    lines.push(`还有 ${summary.value.ocr_pending} 个二维码结果待人工确认`)
  }
