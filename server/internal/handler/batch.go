...
// Audit returns the batch audit trail. The legacy audit table is optional in
// local installations; keep the endpoint available with an empty result.
func (h *BatchHandler) Audit(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"batch_id": c.Param("id"), "items": []any{}})
}
