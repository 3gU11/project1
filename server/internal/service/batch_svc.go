...
func (s *BatchSvc) Confirm(batchID string, actor string, batchCode *string, inboundDate *time.Time) error {
	...
	// HTTP clients may retry after the confirmation commit succeeds but before
	// they receive its response. Treat the same confirmed values as a no-op;
	// reject a conflicting retry so it cannot silently rewrite a frozen plan.
	if b.Status == model.StatusConfirmed {
		if err := confirmedRequestMatches(b, batchCode, inboundDate); err != nil {
			return err
		}
	...
	if err := s.logTx(tx, actor, "confirm", "batch", batchID, map[string]string{"model_type": b.ModelType}); err != nil {
		return err
	}
...
// confirmedRequestMatches defines the idempotency contract for confirmation
// retries. Omitted optional fields do not mutate a frozen batch; supplied
// values must be exactly the ones used by the original confirmation.
func confirmedRequestMatches(batch *model.Batch, batchCode *string, inboundDate *time.Time) error {
	if batchCode != nil && strings.TrimSpace(derefString(batch.BatchCode)) != strings.TrimSpace(*batchCode) {
		return fmt.Errorf("batch already confirmed with a different batch_code")
	}
	if inboundDate != nil {
		if batch.ExpectedInboundDate == nil || !batch.ExpectedInboundDate.Equal(*inboundDate) {
			return fmt.Errorf("batch already confirmed with a different expected_inbound_date")
		}
	}
	return nil
}
...
func (s *BatchSvc) Revoke(batchID string, actor string) error {
	...
	if err := s.logTx(tx, actor, "revoke", "batch", batchID, map[string]string{"model_type": b.ModelType}); err != nil {
		return err
	}
...
func (s *BatchSvc) SyncStockModels(batchID string, targets []StockModelTarget, actor string) error {
	...
	if err := CaptureSandboxBaselines(tx, actor, batchID); err != nil {
	...
	if err := s.logTx(tx, actor, "sync_stock_models", "batch", batchID, map[string]string{
...
func (s *BatchSvc) log(actor string, action string, targetType string, targetID string, detail map[string]string) {
	_ = s.logTx(s.db, actor, action, targetType, targetID, detail)
}

func (s *BatchSvc) logTx(tx *gorm.DB, actor string, action string, targetType string, targetID string, detail map[string]string) error {
