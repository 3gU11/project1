...
def bulk_import():
    ...
    try:
        ...
        # 1. Get top 15 batches
        cursor.execute("""
            SELECT `批次号` 
            FROM finished_goods_data 
            WHERE `批次号` IS NOT NULL AND `批次号` != '' 
            GROUP BY `批次号` 
            ORDER BY MAX(`更新时间`) DESC 
            LIMIT 15
        """)
        batches = [row[0] for row in cursor.fetchall()]
        
        # 2. Get production lines
        cursor.execute("SELECT line_id FROM production_lines ORDER BY display_order LIMIT 15")
        lines = [row[0] for row in cursor.fetchall()]
        
        print(f"Found {len(batches)} batches and {len(lines)} lines.")

        for i, batch_code in enumerate(batches):
            if i >= len(lines):
                break
            
            target_line_id = lines[i]
            ...
            print(f"[{i+1}/15] Syncing batch {batch_code} to {target_line_id}...")
            ...
            # Import units with COLLATE fix and escaped %
            import_sql = """
            INSERT INTO units (
