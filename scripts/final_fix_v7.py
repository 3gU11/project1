...
def final_fix_v7():
    ...
    # Use COALESCE to prioritize fg.合同备注 but fall back to fg.订单备注 if empty
    # This addresses the case like 96-03-407 where remark is in 订单备注
    ...
    SELECT
        ...
        COALESCE(NULLIF(TRIM(fg.`合同备注`), ''), NULLIF(TRIM(fg.`订单备注`), ''), fg.`机台备注/配置`) AS order_remark,
